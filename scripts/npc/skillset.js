...
function action(mode, type, selection) {
    ...
    if (status == 0) {
        ...
        let count = 0;
        let lastSelect; //variable to store selected skill
        let lastKey; //variable to store retrieved keymap
        let chosenKey; //variable for key to map skill to
        for (let i = 0; i < playerSkills.length; i++) {
            ...
            let checkActive = String(skillId).split('')[3];

            // print("skill: " + SkillFactory.getSkillName(skillId) + " / activeSkills=1: " + checkActive);
            if (skillLevel > 0 && checkActive == 1) {
                strbuilder += "#L" + skillId + "##s" + skillId + "# #b#q" + skillId + "#-" + skillLevel + "\t#l";
                count += 1;
            }
            if (count % 2 === 0 && count !== 0 && checkActive == 1) {
                strbuilder += "\r\n";
            }
        }
        if (count > 0) {
            ...
            cm.dispose();
            cm.getPlayer().yellowMessage("[No active skills found!]");
        }

    } else if (status == 1) {
        // print("STATUS:", status, " TYPE:", type, " MODE:", mode, " SELECTION: ", selection);
        if (selection !== -1) {
            lastSelect = selection; //save skill to var as selection gets overwritten on next dialog
            ...
            cm.sendSimple("Which key do you want #s" + lastSelect + "# on? #b\r\n#L59#F1#L60#F2#L61#F3#L62#F4#L63#F5#L64#F6#L65#F7#L66#F8#L67#F9 \r\n #L68#F10#L87#F11#L88#F12 \r\n#L2#1#L3#2#L4#3#L5#4#L6#5#L7#6#L8#7#L9#8#L10#9#L11#0#L12#-#L13#= \r\n#L16#Q#L17#W#L18#E#L19#R#L20#T#L21#Y#L22#U#L23#I#L24#O#L25#P#L26#[#L27#] \r\n#L30#A#L31#S#L32#D#L33#F#L34#G#L35#H#L36#J#L37#K#L38#L#L39#;#L40#' \r\n#L42#Shift#L44#Z#L45#X#L46#C#L47#V#L48#B#L49#N#L50#M#L51#,#L52#.#L42#Shift \r\n#L29#Ctrl#L56#Alt#L57#SPACE#L56#Alt#L29#Ctrl \r\n#L82#Ins#L71#Hm#L73#Pup#L83#Del#L79#End#L81#Pdn");
        } else if (selection == -1 && mode == -1) {
            cm.dispose();
            }
        
        } else if (status == 2) {
        lastKey = cm.getPlayer().getKeymap().get(selection);
        print("currentKey:", lastKey);
        print("lastSelect:", lastSelect);
        print("key: ", selection);
        chosenKey = selection;
        if (lastKey !== null) {
            cm.sendYesNo("The key is currently in use, override?");
        } else {
            cm.getPlayer().changeKeybinding(selection, 1, lastSelect);
            cm.getPlayer().sendKeymap();
            cm.getPlayer().yellowMessage("[Mapped " + SkillFactory.getSkillName(lastSelect) + " for " + cm.getPlayer().getName() + " to key. ]");
            cm.dispose();
        }
        
    } else if (status == 3) {
        cm.getPlayer().changeKeybinding(chosenKey, 1, lastSelect);
        cm.getPlayer().sendKeymap();
        cm.getPlayer().yellowMessage("[Mapped " + SkillFactory.getSkillName(lastSelect) + " for " + cm.getPlayer().getName() + " to key. ]");
        cm.dispose();
    }
